import React, { useState } from 'react';
import { SITE_CONTENT } from '../../constants/content';
import './FloatingContact.css';

const FloatingContact = () => {
  const [showBranches, setShowBranches] = useState(false);
  const [activeAction, setActiveAction] = useState(null); // 'whatsapp' or 'phone'

  const handleActionClick = (e, action) => {
    e.preventDefault();
    if (activeAction === action && showBranches) {
      setShowBranches(false);
      setActiveAction(null);
    } else {
      setActiveAction(action);
      setShowBranches(true);
    }
  };

  const logContact = async (location, action) => {
    try {
      await fetch(`${SITE_CONTENT.api.base}/api/whatsapp-contacts/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          product_name: action === 'whatsapp' ? 'WhatsApp Chat' : 'Phone Call',
          branch: location.name 
        })
      });
    } catch (err) {
      console.error("Failed to log contact:", err);
    }
  };

  const handleBranchSelect = (location) => {
    logContact(location, activeAction);
    if (activeAction === 'whatsapp') {
      window.open(`https://wa.me/${location.whatsapp}`, '_blank');
    } else {
      window.location.href = `tel:${location.phone.replace(/\s/g, '')}`;
    }
    setShowBranches(false);
    setActiveAction(null);
  };
  return (
    <div className="sticky-contact-wrapper">

      {/* BRANCH SELECTION MENU */}
      {showBranches && (
        <div className="branch-selector-menu">
          <p className="menu-title">
            Which branch would you like to {activeAction === 'whatsapp' ? 'Message' : 'Call'}?
          </p>
          <div className="branch-options">
            {SITE_CONTENT.locations.map(loc => (
              <button 
                key={loc.id} 
                className="branch-opt-btn"
                onClick={() => handleBranchSelect(loc)}
              >
                {loc.name}
              </button>
            ))}
          </div>
        </div>
      )}

      {/* WHATSAPP BUTTON */}
      <button
        onClick={(e) => handleActionClick(e, 'whatsapp')}
        className={`contact-bubble-btn whatsapp-action ${activeAction === 'whatsapp' ? 'active' : ''}`}
        aria-label="WhatsApp Us"
        style={{ backgroundColor: '#25D366', display: 'flex' }}
      >
        <svg width="32" height="32" viewBox="0 0 24 24" fill="white" xmlns="http://www.w3.org/2000/svg">
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L0 24l6.335-1.662c1.72.94 3.659 1.437 5.634 1.437h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
        </svg>
      </button>

      {/* PHONE BUTTON */}
      <button
        onClick={(e) => handleActionClick(e, 'phone')}
        className={`contact-bubble-btn phone-action ${activeAction === 'phone' ? 'active' : ''}`}
        aria-label="Call Us"
        style={{ backgroundColor: '#004b8d', display: 'flex' }}
      >
        <svg width="28" height="28" viewBox="0 0 24 24" fill="white" xmlns="http://www.w3.org/2000/svg">
          <path d="M20 15.5c-1.2 0-2.4-.2-3.6-.6-.3-.1-.7 0-1 .2l-2.2 2.2c-2.8-1.4-5.1-3.8-6.6-6.6l2.2-2.2c.3-.3.4-.7.2-1-.3-1.1-.5-2.3-.5-3.5 0-.6-.4-1-1-1H4c-.6 0-1 .4-1 1 0 9.4 7.6 17 17 17 .6 0 1-.4 1-1v-3.5c0-.6-.4-1-1-1zM18 7h2c.6 0 1-.4 1-1s-.4-1-1-1h-2c-.6 0-1 .4-1 1s-.4 1 1 1zm-3 4h5c.6 0 1-.4 1-1s-.4-1-1-1h-5c-.6 0-1 .4-1 1s-.4 1 1 1z" />
        </svg>
      </button>

    </div>
  );
};

export default FloatingContact;
